...
import { Clock, Calendar, ChevronLeft, CalendarCheck } from 'lucide-react';
...
export default function ServiceDetail() {
  ...
  const { data: serviceData, isLoading } = useQuery({
    queryKey: ['service', id],
    queryFn: () => serviceAPI.getById(id!).then((r) => r.data),
    enabled: !!id,
  });
  ...
  const WALLET_DISCOUNT_RATES: Record<number, number> = { 2: 0.1, 3: 0.12, 4: 0.15, 5: 0.2 };
  const walletDiscountRate = user ? WALLET_DISCOUNT_RATES[user.customerRanking] || 0 : 0;
  const walletPrice = walletDiscountRate > 0 ? Math.round(basePrice * (1 - walletDiscountRate) * 100) / 100 : basePrice;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid lg:grid-cols-1 gap-8">
        ...
        <div>
          <span className="inline-block text-xs font-medium text-gold-600 bg-gold-50 px-3 py-1 rounded-full mb-3">
            {service?.category}
          </span>
          ...
          <div className="flex items-center gap-6 mb-6 pb-6 border-b border-gray-100">
            {service?.duration ? (
              <div className="flex items-center gap-2 text-gray-600">
                ...
                <span>{service?.duration} {t('servicesPage.minutes')}</span>
              </div>
            ) : null}
            <div className="text-2xl font-bold text-rose-deep">RM{service?.price}</div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {user && (
              <div className="space-y-2">
                ...
                <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'wallet' ? 'border-rose-deep bg-rose-soft' : 'border-gray-200'}`}>
                  ...
                  <div>
                    ...
                    {walletDiscountRate > 0 && (
                      <p className="text-xs text-gold-600 font-medium">{t('servicesPage.walletDiscountRank', { stars: user?.customerRanking, percent: Math.round(walletDiscountRate * 100), amount: walletPrice.toFixed(2) })}</p>
                    )}
                ...
                <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'split' ? 'border-rose-deep bg-rose-soft' : 'border-gray-200'}`}>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-700">{user?.customerRanking === 1 ? t('servicesPage.splitPayment1Star') : t('servicesPage.splitPayment')}</p>
                    <p className="text-xs text-gray-400">{t(user?.customerRanking === 1 ? 'servicesPage.splitPaymentDesc1Star' : 'servicesPage.splitPaymentDesc', { balance: (user.walletBalance || 0).toFixed(2), bonus: (user.walletBonus || 0).toFixed(2) })}</p>
                  </div>
                </label>
              </div>
            )}
            ...
            <Button
              ...
              size="lg"
            >
              {createBooking.isPending ? t('servicesPage.booking') : t('servicesPage.bookNow')}
            </Button>
          </div>
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        title={t('confirmModal.booking.title')}
        message={t('confirmModal.booking.message')}
        icon={<CalendarCheck size={22} className="text-rose-deep" />}
        confirmLabel={t('confirmModal.booking.confirm')}
        cancelLabel={t('confirmModal.cancel')}
        processingLabel={t('confirmModal.processing')}
        isLoading={createBooking.isPending}
        details={[
          { label: t('confirmModal.service'), value: service?.name },
          ...
          },
          {
            ...
            value: (
              <span className="text-rose-deep font-bold">
                RM{(paymentMethod === 'wallet' ? walletPrice : basePrice).toFixed(2)}
              </span>
            ),
          },
        ]}
